...
FIELD_GAP_RULES: dict[str, tuple[list[str], list[str]]] = {
    "major": (
        ...
        [
            ...
            # 2026-08-12 추가 — "체육특기자 전형 입학생 대상"처럼 "전공"/"학과"라는 단어 자체를
            # 안 쓰고 입학전형(특기자 전형)으로만 조건을 표현하는 경우를 놓치고 있었음(major=NULL,
            # required_special_status=[] 둘 다 비어서 사실상 전교생에게 노출되던 사고, 실사용자
            # UX 리서치에서 "인문대생인데 체육특기자 장학금이 뜬다"로 발견됨). 실제로는 "OO특기자
            # 전형" 조건이 major 필드로 완전히 정확하게 표현되진 않지만(학과가 아니라 입학전형
            # 기준이라 관련 학과 학생이 아니어도 특기자 전형 입학생일 수 있음), 최소한 전혀
            # 무관한 학과 학생에게까지 노출되는 것보다는 관련 학과로 좁히는 게 훨씬 나음
            # (과다매칭이 과소매칭보다 낫다는 이 프로젝트의 기존 원칙과 같은 방향).
            r"체육특기", r"예능특기", r"[가-힣]{1,6}특기자\s*(전형|대상)",
        ],
    ),
