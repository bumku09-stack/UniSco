...
**지금 당장 나이트런을 켜도 아무 대학도 처리되지 않음.** `frontend/src/lib/universities.ts`엔
단과대/학과 조직도만 있고, 실제 장학공지 게시판 URL·페이지네이션 구조·HTML 셀렉터는 이
프로젝트 어디에도 검증된 형태로 기록돼 있지 않아서 — 검증 안 된 셀렉터를 미리 채워 넣는 건
이 하네스가 막으려는 문제(확인 안 된 걸 확인된 것처럼 다루는 것)를 설정 단계에서 저지르는
꼴이라 비워뒀음.
